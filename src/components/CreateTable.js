      ...
      <div>
        {
          (() =>{
            let list = [];
            if(isAddable) list.push(<DialogForm key="0" formType="insert" formTitle={insertFormTitle} isOpen={this.state.insertFormIsOpen} closeCallback={this.closeCallback} crudFormInfo={crudFormInfo} getData={this.getInsertFormData} _onChange={this.props.onChange} />);
            if(isEditable) list.push(<DialogForm key="1" formType="update" formTitle={updateFormTitle} isOpen={this.state.updateFormIsOpen} closeCallback={this.closeCallback} crudFormInfo={crudFormInfo} selectedData={this.state.selectedData} getData={this.getUpdateFormData} _onChange={this.props.onChange} />);

            return list;
          })()
        }
        <DataTable key={this.props._key} tableData={tableData} loading={isFetching} isCheckable={isCheckable} columns={columns} isAddable={isAddable} isEditable={isEditable} isRemovable={isRemovable} insertClick={this.insertClick} updateClick={this.updateClick} deleteClick={this.deleteClick} defaultSorted={defaultSorted} defaultPageSize={defaultPageSize} />
      </div>
    );
  }
}
