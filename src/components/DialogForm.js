...
function createToggle(content, key, setValidateFunc, setValueFunc, formType, selectedData, isStepper, _onChange) {
    ...
    return <DialogSwitch key={key} index={key} dbColumnName={dbColumnName} initialValue={_selectedData} label={label}
                         defaultValue={content.defaultValue} setValidateFunc={setValidateFunc}
...
class DialogForm extends React.Component {
            ...
            _.map(crudFormInfo, (item, index) => {
                ...
                if (isInclude) {
                    ...
                    switch (type) {
                        case "text":
                        case "textfield":
                            list.push(<Grid item xs={xs}
                                            key={index}>{createText(item, index, setValidateFunc, setValueFunc, formType, selectedData, isStepper, _onChange)}</Grid>);
                            break;
                        case "datepicker":
                            list.push(<Grid item xs={xs}
                                            key={index}>{createDatePicker(item, index, setValidateFunc, setValueFunc, formType, selectedData, isStepper, _onChange)}</Grid>);
                            break;
                        case "dropdown":
                            list.push(<Grid item xs={xs}
                                            key={index}>{createDropDown(item, index, setValidateFunc, setValueFunc, formType, selectedData, isStepper, _onChange)}</Grid>);
                            break;
                        case "toggle":
                            list.push(<Grid item xs={xs}
                                            key={index}>{createToggle(item, index, setValidateFunc, setValueFunc, formType, selectedData, isStepper, _onChange)}</Grid>);
                            break;
                        case "upload":
                            list.push(<Grid item xs={xs}
                                            key={index}>{createUpload(item, index, setValidateFunc, setValueFunc, formType, selectedData, isStepper, valueList, _onChange)}</Grid>);
                            break;
                        case "searchdropdown":
                            list.push(<Grid item xs={xs}
                                            key={index}>{createSearchDropDown(item, index, setValidateFunc, setValueFunc, formType, selectedData, isStepper, _onChange)}</Grid>);
                            break;
                        default :
                            break;
                    }
    ...
    render() {
        ...
        const actions = [
            ...
            <Button key={1} primary={"true"} disabled={!this.state.isValidate}
